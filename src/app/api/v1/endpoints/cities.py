...
from app.models import CitiesModel, UserModel
from app.schemas.citySchema import CitiesSchemaBase, CitiesSchemaCreate, CitiesSchemaWithRelations, CitiesFilters, CitiesSchemaResponse, CitiesSchemaWithRelationsResponse
...
from app.utils.exceptions import UniqueViolationException, NotFoundException, UnauthorizedException
...
async def create(data: CitiesSchemaCreate,
    ...
    O endpoint requer:
    - id (codigo IBGE) (unico)
    - name (nome da cidade) 
    - state_id (id do estado ao qual pertence a cidade)
    
    Somente o campo (id) é unico, mas os 
...
@router.get("/{id}", 
            ...
            status_code=status.HTTP_200_OK,
            response_model=CitiesSchemaWithRelationsResponse,
            responses={
                status.HTTP_500_INTERNAL_SERVER_ERROR: {
                    "model": InternalServerResponse
                },
